...
**Table I** and Fig. 3 provide the visualization of the overall runtime performance comparison. We assumed that 32-bit softwares, like VBA, would only be affected slightly by computer specs, but our results revealed that computer specs also played a significant role in boosting the overal performance of code refactoring and could further our efficiency and ease certain challenges during the code refactoring process. Being unaware of this limitation was a big mistake when trying to beat the record runtime, which triggered rounds and rounds of experiments and redoing the code refactoring. However, this project has taught us a lot of lessons, and conducting additional researches and measurements are often required to solve problems, even when our initial results turned out to slightly off.
...
- Disadvantages include:
  - time-consuming tasks to complete effective code refactoring, though the outcomes may not be easily distinguished.
